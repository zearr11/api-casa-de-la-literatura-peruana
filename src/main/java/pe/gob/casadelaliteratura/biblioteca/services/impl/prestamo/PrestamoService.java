...
public class PrestamoService implements IPrestamoService {
    ...
    public MensajeDto<String> registrarPrestamo(PrestamoRequestDto datosPrestamo) {
        ...
        Prestamo nuevoPrestamo = new Prestamo(acService.generateCodigo("PS"), LocalDate.now(),
                ...
                detalleUsuarioService.obtenerUsuarioAutenticado());
    ...
    public MensajeDto<String> renovarPrestamo(RenovacionRequestDto datosRenovacion) {
        ...
        Prestamo prestamo = prestamoRepository.findById(datosRenovacion.getCodPrestamo()).orElse(null);
        List<Renovacion> renovaciones = renovacionRepository.findByPrestamo(prestamo);

        if (prestamo == null)
            throw new ErrorException404("Préstamo con código: " + datosRenovacion.getCodPrestamo() + " no existe.");

        if (prestamo.getEstadoDevolucion().equals(EstadoDevolucion.DEVOLUCION_PARCIAL))
            throw new ErrorException409(
                    "El préstamo se encuentra con el estado de devolución parcial. En este estado no es posible realizar renovaciones."
            );

        if (prestamo.getEstadoDevolucion().equals(EstadoDevolucion.DEVOLUCION_COMPLETA))
            throw new ErrorException409(
                    "El préstamo ya está registrado como entregado."
            );

        if (renovaciones.size() == 3)
            throw new ErrorException409(
                    "El préstamo ya cuenta con 3 renovaciones. No es posible aplazar por mas dias."
            );

        LocalDate ultimaFechaVenc = prestamo.getFechaVencimiento();

        if (!renovaciones.isEmpty()){
            for (Renovacion obj : renovaciones) {
                if (obj.getNuevaFechaVencimiento().isAfter(ultimaFechaVenc))
                    ultimaFechaVenc = obj.getNuevaFechaVencimiento();
            }
        }

        long diasRestantes = ChronoUnit.DAYS.between(LocalDate.now(), ultimaFechaVenc);

        if (!ultimaFechaVenc.isAfter(LocalDate.now()))
            throw new ErrorException409("El préstamo está vencido, ya no es posible realizar una renovación.");

        if (diasRestantes > 7)
            throw new ErrorException409(
                    "La solicitud de renovación no puede ser atendida ahora mismo. " +
                            "Puede solicitar una renovación del préstamo a partir del " +
                            ultimaFechaVenc.minusDays(7) +
                            " (7 días antes del vencimiento: " + ultimaFechaVenc + ")"
            );

        Renovacion renovacion = new Renovacion();
        renovacion.setPrestamo(prestamo);
        renovacion.setUsuario(detalleUsuarioService.obtenerUsuarioAutenticado());
        ...
        return new MensajeDto<>("Renovación registrada.");
    }
